...
class FibHeap
{
  ...
  */
  ...
  void delete_fibnodes(FibNode<T1,T2> *x)
  {
    if (!x)
      return;
   ...
   *12. return z
   */
  FibNode<T1,T2>* extract_min()
  {
    FibNode<T1,T2> *z, *x, *next;
    FibNode<T1,T2> ** childList;
	...
	if ( x != nullptr )
	  {
	    childList = new FibNode<T1,T2>*[z->degree];
	    next = x;
  ...
  void consolidate()
  {
    FibNode<T1,T2>* w, * next, * x, * y, * temp;
    FibNode<T1,T2>** A, ** rootList;
    ...
    // 1
    A = new FibNode<T1,T2>*[max_degree+2]; // plus two both for indexing to max degree and so A[max_degree+1] == NIL
    // 2, 3
    ...
    rootList = new FibNode<T1,T2>*[rootSize];
 ...
 * 3. y.mark = FALSE
 */
  void fib_heap_link( FibNode<T1,T2>* y, FibNode<T1,T2>* x )
  {
    // 1
   ...
   * 9. 	H.min = x
   */
  void decrease_key( FibNode<T1,T2>* x, int k )
  {
    FibNode<T1,T2>* y;
   ...
   * 4. x.mark = FALSE
   */
  void cut( FibNode<T1,T2>* x, FibNode<T1,T2>* y )
  {
    // 1
    if ( x->right == x )
      {
...
/*
  ...
  * 6. 		CASCADING-CUT(H,z)
  */
  void cascading_cut( FibNode<T1,T2>* y )
  {
    FibNode<T1,T2>* z;
   ...
   */
  void remove_fibnode( FibNode<T1,T2>* x )
  {
    decrease_key(x,std::numeric_limits<T1>::min());
    FibNode<T1,T2> *fn = extract_min();
    delete fn;
  }
  ...
  void pop()
  {
    if (empty())
      return;
    FibNode<T1,T2> *x = extract_min();
    if (x)
      delete x;
  }

  FibNode<T1,T2>* push(T1 k, T2 v, void *pl)
  {
    FibNode<T1,T2> *x = new FibNode<T1,T2>(k,v,pl);
    insert(x);
    return x;
  }

  FibNode<T1,T2>* push(T1 k, T2 v)
  {
    return push(k,v,nullptr);
  }
